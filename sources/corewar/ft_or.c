...
void	ft_or_do_with_arg1(t_war *war, t_crg *crg,
		int *pos, int *bit_or_arg1_2)
{
	int tmp_arg;
	int new_pos;
...
void	ft_or(t_war *war, t_crg *crg)
{
	...
	pos = crg->pos + 2;
	bit_or_arg1_2 = 0;
	ft_and_or_xor_do_with_arg0(war, crg, &pos, &bit_or_arg1_2);
	ft_or_do_with_arg1(war, crg, &pos, &bit_or_arg1_2);
	tmp_arg = war->arena[GG(pos)].code;
	if (tmp_arg >= 1 && tmp_arg <= 16)
	{
